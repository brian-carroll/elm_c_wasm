module Core.Array exposing (tests)
...
> threeLevelTree elements requires 3 levels in the tree

-}
defaultSizeRange : Fuzzer Int
defaultSizeRange =
    intRange 1 threeLevelTree
...
lengthTests =
    describe "Length"
        ...
        --             |> Expect.equal size
        -- , fuzz (intRange smallSlice bigSlice) "big slice" <|
        --     \size ->
        --         length (slice 35 -35 (initialize size identity))
        --             |> Expect.equal (size - 70)
        -- , fuzz2 (intRange -32 -1) (intRange smallSlice bigSlice) "small slice end" <|
        --     \n size ->
...
getSetTests =
    describe "Get and set"
        [ -- fuzz2 defaultSizeRange defaultSizeRange "can retrieve element" <|
          ...
          --             |> Expect.equal (Just n)
          -- , fuzz2 (intRange 1 smallerSlice) (intRange smallSlice threeLevelTree) "out of bounds retrieval returns nothing" <|
          --     \n size ->
          --         let
          --             arr =
          ...
          --             |> Expect.equal (Just 5)
          -- , fuzz2 (intRange 1 smallerSlice) defaultSizeRange "set out of bounds returns original array" <|
          --     \n size ->
          --         let
          --             arr =
          --                 initialize size identity
...
sliceTests =
    ...
    in
    describe "Slice"
        [ -- fuzz2 (intRange -50 -1) (intRange smallSlice threeLevelTree) "both" <|
          -- \n size ->
          -- slice (abs n) n (initialize size identity)
          -- |> Expect.equal (initialize (size + n + n) (\idx -> idx - n))
          -- , fuzz2 (intRange -50 -1) (intRange smallSlice threeLevelTree) "left" <|
          -- \n size ->
          -- let
          -- arr =
          -- initialize size identity
          ...
          -- , fuzz2 (intRange -50 -1) (intRange smallSlice threeLevelTree) "right" <|
          -- \n size ->
