...
void* GC_stack_empty() {
  ...
  bool is_already_empty = state->stack_map == state->stack_map_empty &&
                          state->stack_map_empty->header.tag == Tag_GcStackEmpty &&
                          state->next_alloc == ((size_t*)state->stack_map_empty +
                                                   sizeof(GcStackMap) / SIZE_UNIT);
  if (is_already_empty) {
