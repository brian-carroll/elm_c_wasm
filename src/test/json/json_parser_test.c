...
void* test_Json_parse_number() {
  parse_test(&parse_number, create_string("123.456"), ctorElmFloat(123.456), 7);
  parse_test(&parse_number, create_string("-123.456"), ctorElmFloat(-123.456), 8);
  parse_test(&parse_number, create_string("-123.456e-3"), ctorElmFloat(-123.456e-3), 11);
  parse_test(&parse_number, create_string("-123.456E+1"), ctorElmFloat(-1234.56), 11);
...
void* test_Json_parse_array() {
  ...
  parse_test(&parse_array,
      create_string("[123]"),
      ctorCustom(JSON_VALUE_ARRAY, 1, (void*[]){ctorElmFloat(123)}),
      5);

  parse_test(&parse_array,
      create_string("[\r\n\tnull\r\n ] "),
      ctorCustom(JSON_VALUE_ARRAY, 1, (void*[]){&Json_encodeNull}),
      12);

  parse_test(&parse_array,
      create_string("[ 123,\"hi\" ]"),
      ctorCustom(JSON_VALUE_ARRAY,
          2,
          ((void*[]){
              ctorElmFloat(123),
              create_string("hi"),
  ...
  parse_test(&parse_array, create_string("[]"), ctorCustom(JSON_VALUE_ARRAY, 0, NULL), 2);

  parse_test(
      &parse_array, create_string("[ \r\n\t]"), ctorCustom(JSON_VALUE_ARRAY, 0, NULL), 6);
...
void* test_Json_parse_object() {
  ...
  parse_test(
      &parse_object, create_string("{}"), ctorCustom(JSON_VALUE_OBJECT, 0, NULL), 2);

  parse_test(&parse_object,
      create_string("{ \r\n\t}"),
      ctorCustom(JSON_VALUE_OBJECT, 0, NULL),
      6);
