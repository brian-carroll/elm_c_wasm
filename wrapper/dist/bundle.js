...
var author$project$Main$cmdTime = A2(elm$core$Task$perform, author$project$Main$GotTime, elm$time$Time$now);
...
var author$project$WasmWrapper$element = elm$browser$Browser$element;
...
function wrapWasmElmApp(wasmBuffer, wasmExports, generatedAppTypes, kernelFunctions) {
    ...
    function readWasmValue(addr) {
        ...
        switch (tag) {
            ...
            case Tag.Custom: {
                ...
                var custom = { $: jsCtor };
                var fieldCharCode = 'a'.charCodeAt(0);
                for (var i = index + 2; i < index + size; i++) {
                    var field = String.fromCharCode(fieldCharCode++);
            ...
            case Tag.Record: {
                ...
                for (var i = 1; i <= fgSize; i++) {
                    ...
                    var fieldName = appTypes.fields[fieldId];
    ...
    function createWasmCallback(metadata) {
        ...
        function wasmCallback() {
            ...
            var resultAddr = wasmExports._callClosure(addr);
     ...
     * May throw an error
     */
    function writeWasmValue(value) {
        return handleWasmWrite(function (nextIndex) { return writeWasmValueHelp(nextIndex, value); });
    }
    function writeWasmValueHelp(nextIndex, value) {
    ...
    function writeFromBuilder(nextIndex, builder, tag) {
        if (builder.bodyWriter) {
            ...
            jsChildren.forEach(function (child) {
                var update = writeWasmValueHelp(nextIndex, child); // recurse
    ...
    var mainRecordAddr = wasmExports._getMainRecord();
    var mainRecord = mainRecordAddr
